...
const EventForm = ({ selectedDate, onClose, existingEvent }) => {
    ...
    const [date, setDate] = useState(existingEvent ? existingEvent.date : selectedDate.toISOString().split('T')[0]);
    const [hour, setHour] = useState(existingEvent ? existingEvent.hour : '12');
    const [minute, setMinute] = useState(existingEvent ? existingEvent.minute : '00');
